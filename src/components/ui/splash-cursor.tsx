...
function SplashCursor({
  ...
  useEffect(() => {
    ...
    function getWebGLContext(canvas: HTMLCanvasElement) {
      ...
      let glRaw = canvas.getContext("webgl2", params);
      let gl: WebGLRenderingContext | WebGL2RenderingContext | null =
        glRaw && "getParameter" in glRaw
          ? (glRaw as WebGLRenderingContext | WebGL2RenderingContext)
          : null;
      let isWebGL2 = !!gl;
      if (!isWebGL2) {
        const glFallback =
        ...
        gl =
          glFallback && "getParameter" in glFallback
            ? (glFallback as WebGLRenderingContext)
            : null;
      }
      if (!gl) return null;
      let halfFloat: any;
    ...
    function createProgram(
      ...
      if (!vertexShader || !fragmentShader) {
        throw new Error("Shader não pode ser nulo");
      }
    ...
    function initFramebuffers() {
      ...
      if (!dye)
        dye = createDoubleFBO(
          dyeRes.width,
          dyeRes.height,
          rgba ? rgba.internalFormat : gl.RGBA,
          rgba ? rgba.format : gl.RGBA,
          texType,
          filtering
        );
      else
      ...
      if (!velocity)
        velocity = createDoubleFBO(
          simRes.width,
          simRes.height,
          rg ? rg.internalFormat : gl.RGBA,
          rg ? rg.format : gl.RGBA,
          texType,
          filtering
        );
      else
    ...
    function splat(
      ...
      gl.uniform1f(
        ...
        (canvas ? canvas.width : 1) / (canvas ? canvas.height : 1)
      );
    ...
    function correctDeltaX(delta: number) {
      let aspectRatio = canvas ? canvas.width / canvas.height : 1;
